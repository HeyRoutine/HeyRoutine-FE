...
const UniversityScreen = ({ navigation, route }: any) => {
  ...
  const { data: searchResults, isLoading: isSearching } =
    useGetUniversities(searchKeyword);
  ...
  const handleNext = () => {
    // 대학교 정보를 스토어에 저장
    setSignupUniversity(university);
  ...
  return (
    <Container>
      <Header
        ...
        rightComponent={<ProgressText>4/6</ProgressText>}
      />

      <Content>
        <Title>
          현재 다니시는 <HighlightText>대학교</HighlightText>를{'\n'}
          입력해주세요.
        </Title>
        ...
        {/* 검색 결과 리스트 */}
        {showSearchResults && university.length > 0 && (
